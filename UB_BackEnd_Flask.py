...
ub_app = Flask( __name__ )

origins = [ "http://localhost", "http://localhost:3000" ]

CORS( ub_app, resources = { r"/*": { "origins": origins } } )

"""The below root route is just for testing purposes, so commented out.
@ub_app.route( "/", methods = [ "GET" ] )
def testing():
    return { "msg": "great!" }
"""

@ub_app.route( "/add_customer", methods = [ "POST" ] )
def add_cust():
...
@ub_app.route( "/view_trans", methods = [ "POST" ] )
def view_trans():
    parameters = request.json
...
@ub_app.route( "/deactivate_customer", methods = [ "POST" ] )
def deactivate_cust():
    parameters = request.json
    if parameters["type"] != "mgr":
        return "Need higher level access to deactivate customer. Contact manager."  
    db_conn = sqlite3.connect("UB.db")
...
@ub_app.route( "/view_customer", methods = [ "POST" ] )
def view_cust():
    parameters = request.json
    if parameters["type"] != "mgr":
        return "Need higher level access to view customer details. Contact manager."  
...
@ub_app.route( "/view_customers", methods = [ "GET" ] )
def view_customers():
...
@ub_app.route( "/modify_customer", methods = [ "POST" ] )
def modify_cust():
    parameters = request.json
    if parameters["type"] != "mgr":
...
@ub_app.route( "/validate_password", methods = [ "POST" ] )
...
@ub_app.route( "/manager_login", methods = [ "POST" ] )
def manager_login():
    parameters = request.json
...
@ub_app.route( "/balance_enquiry", methods = [ "POST" ] )
def balance_enquiry():
    parameters = request.json
    ctr_id = parameters[ "ctr_id" ]
...
@ub_app.route( "/balance_change", methods = [ "POST" ] )
def balance_change():
    parameters = request.json
    ctr_id = parameters[ "ctr_id" ]
...
@ub_app.route( "/check_customer_id", methods = [ "POST" ] )
